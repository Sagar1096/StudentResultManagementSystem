...
class reportClass:
    ...
    #================title===================
       ...
       title=Label(self.root,text="View Studnt Result ",font=("goudy old style",20,"bold"),bg="orange",fg="#262626").place(x=10,y=15,width=1180,height=50)       
    ...
    def delete(self):
      ...
      try:
         if self.var_id=="":
            ...
            cur.execute("select * from result where roll=?",(self.var_id))
            row=cur.fetchone()
            if row==None:
               ...
               if op==True:
                  cur.execute("delete from result where roll=?",(self.var_id))
                  con.commit()
